    ...
    public ThanhToanResponse thanhToanDonHang(Long donHangID, ThanhToanRequest request) {
        HoaDon hoaDon = hoaDonRepository.findByDonHang_DonHangID(donHangID)
                .orElseThrow(() -> new ResourceNotFoundException(
                        "Khong tim thay hoa don cho don hang #" + donHangID));
 
        // ★ CHECK 1: Trạng thái đơn hàng phải hợp lệ để thanh toán
        ...
        if (trangThai == TrangThaiDonHang.CHO_XAC_NHAN) {
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST,
                    "Không thể thanh toán — đơn hàng chưa được xác nhận (đang ở trạng thái: CHO_XAC_NHAN).");
        }
        ...
        if (Double.compare(soTien, soTienPhaiTra) != 0) {
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST,
                    "So tien thanh toan khong khop voi hoa don.");
        }

...
CREATE TABLE Ban (
    ...
    TrangThai   NVARCHAR(30)  DEFAULT N'Trong'
                    CHECK (TrangThai IN (N'Trong', N'Dang dung', N'Dat truoc', N'Bao tri')),
...
-- 3. MON AN (dung Table-Per-Hierarchy: PhanLoai = DO_AN | DO_UONG)
CREATE TABLE MonAn (
...
CREATE TABLE KhachHang (
    ...
    HoTen         NVARCHAR(100) NOT NULL,      -- Java entity dung HoTen
    SDT           VARCHAR(15)   UNIQUE,
    DiaChi        NVARCHAR(255),
    Email         NVARCHAR(100),
    DiemTichLuy   INT           DEFAULT 0 CHECK (DiemTichLuy >= 0),
    HangKhachHang NVARCHAR(20)  DEFAULT N'DONG'
                      CHECK (HangKhachHang IN (N'DONG', N'BAC', N'VANG', N'KIM_CUONG')),
    NgayTao       DATETIME      DEFAULT GETDATE(),
    NgayCapNhat   DATETIME      DEFAULT GETDATE()
);
...
CREATE TABLE NhanVien (
    ...
    Password    NVARCHAR(255)  NOT NULL,       -- Se hash BCrypt sau
    NgayTao     DATETIME       DEFAULT GETDATE(),
    NgayCapNhat DATETIME       DEFAULT GETDATE()
);
...
-- 7. DON HANG
--    TrangThai dong bo voi enum TrangThaiDonHang.java:
--    CHO_XAC_NHAN | DANG_NAU | DA_PHUC_VU | HOAN_THANH | DA_HUY
...
-- 9. KHUYEN MAI
--    LoaiKhuyenMai dong bo voi enum LoaiKhuyenMai.java:
--    PHAN_TRAM | GIAM_TIEN_MAT  (da xoa SO_TIEN)
...
-- 11. THANH TOAN
--    PhuongThuc dong bo voi enum PhuongThucThanhToan.java
--    TrangThai dong bo voi enum TrangThaiThanhToan.java:
...
CREATE TABLE ThanhToan (
    ...
    TrangThai   NVARCHAR(20)   DEFAULT N'THANH_CONG'
...
-- NHAN VIEN
-- Password dang la plaintext '12345' / '123456'
-- Sau khi chay app, goi POST /api/migrate/hash-passwords de hash BCrypt
-- Roi xoa MigrateController.java di
INSERT INTO NhanVien (HoTen, SDT, DiaChi, Email, ChucVu, Luong, TrangThai, Password)
VALUES
    ...
    (N'Nguyen Thi Huong',   '0909999999', N'HCM', 'chuquan@example.com', N'CHU_QUAN',      25000000, 1, '123456');
GO
 
-- KHACH HANG
INSERT INTO KhachHang (HoTen, SDT, Email, DiemTichLuy, HangKhachHang)
VALUES
...
-- KHUYEN MAI
-- LoaiKhuyenMai chi co 2 gia tri hop le: PHAN_TRAM hoac GIAM_TIEN_MAT
-- NgayKetThuc update thanh 2026 de con hieu luc khi chay demo
INSERT INTO KhuyenMai (TenKhuyenMai, LoaiKhuyenMai, GiaTri, NgayBatDau, NgayKetThuc, TrangThai, MoTa)
VALUES
    (N'Giam 10% cuoi tuan',     N'PHAN_TRAM',    10,    '2025-01-01', '2026-12-31', 1, N'Giam 10% cho don hang cuoi tuan'),
    (N'Tang 50k don tren 300k', N'GIAM_TIEN_MAT', 50000, '2025-05-01', '2026-12-31', 1, N'Tang 50k cho don tu 300k'),
    (N'Uu dai thanh vien Vang', N'PHAN_TRAM',    15,    '2025-01-01', '2026-12-31', 1, N'Giam 15% cho hoi vien Vang');
GO
...
SELECT N'KhuyenMai',                COUNT(*) FROM KhuyenMai  UNION ALL
SELECT N'CaLamViec',                COUNT(*) FROM CaLamViec;
 
SELECT KhuyenMaiID, TenKhuyenMai, LoaiKhuyenMai, GiaTri, NgayKetThuc, TrangThai
FROM KhuyenMai;
 
PRINT N'';
PRINT N'===== DONE =====';
PRINT N'Buoc tiep theo: chay app, goi POST /api/migrate/hash-passwords de hash password BCrypt,';
PRINT N'sau do xoa file MigrateController.java.';
GO
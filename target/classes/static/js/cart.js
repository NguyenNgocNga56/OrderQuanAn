  // ============================================================
    ...
    async function loadKhuyenMaiSelect() {
        try {
            ...
            // KhuyenMaiController trả về ApiResponse { data: [...] }
    ...
    function applyKhuyenMai() {
        ...
        if (loai === 'PHAN_TRAM') {
            ...
            _giamGia = Math.min(gia, subtotal); // không giảm vượt tổng đơn
    ...
    async function datMon() {
        ...
        try {
            ...
            if (!ttRes.ok) {
                ...
                throw new Error(err.message || 'Thanh toan that bai');
            }
...
// ===== SCROLL REVEAL =====
(function() {
    const targets = ['.cart-page-items', '.info-card'];
    targets.forEach(sel => {
    ...
    function renderSuccess(order, payment) {
        ...
        const diemMessage = payment?.diemCong > 0
            ? `<p style="text-align:center;margin-top:10px;font-size:0.85rem;color:var(--text-muted);">Da cong ${payment.diemCong} diem sau khi thanh toan.</p>`
            : '';
